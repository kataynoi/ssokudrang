$(document).ready(function () {
    var dataTable = $('#table_data').DataTable({
        ...
        "pageLength": 50,
        "ajax": {
            url: site_url + '/admin_link/fetch_admin_link',
            data: {
...
crud.ajax = {
    del_data: function (id, cb) {
        var url = '/admin_link/del_admin_link',
            params = {
                id: id
            }
        ...
        var url = '/admin_link/save_admin_link',
            params = {
                items: items
            }
        ...
        var url = '/admin_link/get_admin_link',
            params = {
                id: id
            }
...
crud.set_after_update = function (items, row_id) {
    ...
    row_id.find("td:eq(2)").html(items.link);

}
crud.set_after_insert = function (items, id) {

    $('<tr name="row' + (id + 1) + '"><td>' + id + '</td>' +
        '<td>' + items.id + '</td>' + '<td>' + items.name + '</td>' + '<td>' + items.link + '</td>' +
...
function validate(items) {
     ...
     if (!items.name) {
        swal("กรุณาระบุชื่อLink");
        $("#name").focus();
    } else if (!items.link) {
        swal("กรุณาระบุที่อยู่Link");
        $("#link").focus();
    }
    else {
        return true;
